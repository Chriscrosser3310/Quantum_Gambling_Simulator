import pygame
import numpy
from game_lib.monty_hall.SharedClasses import OpenButton, ConfirmButton, BackButton, CircuitButton, TutorialBlock
...
class CheckBob:
    ...
    def __init__(self, pos, data):
        ...
        self.images = pygame.transform.scale(pygame.image.load('assets/images/alice.jpg'),
                                              (self.width, self.height))
    ...
    def draw(self, surface):
        if self.checked:
            surface.blit(self.images, self.rect.topleft)
...
class AliceOpensDoor:
    def __init__(self, data):
        ...
        self.OpenButton = OpenButton((cx, cy * 8 / 5))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT or self.keys[pygame.K_ESCAPE]:
                ...
                if event.button == 1:
                    ...
                    self.OpenButton.check_click(event.pos)
    ...
    def main_loop(self):
        while not (self.quit or self.next_stage or self.back or self.show_circuit):
            ...
            if self.OpenButton.click:
                self.OpenButton.update_click()
                # continue only when bob's choice has been set
                if self.data["BobChosenDoor"] != -1:
                    self.next_stage = True

                    for door in range(len(self.data['BallProbDist'])):
                        self.data['BallProbDist'][door]+=self.data['BallProbDist'][self.data["BobChosenDoor"]]/2
                    self.data['BallProbDist'][self.data["BobChosenDoor"]] = 0
